...
namespace beicode
{
	...
	{
		if (!utf16_is_leading_surrogate_code_point(leading)) return -1;
		if (!utf16_is_trailing_surrogate_code_point(trailing)) return -1;
	...
	namespace
	{
		struct utf8_category_t : std::error_category
		{
	...
	{
		static const utf8_category_t category{};
		return category;
	}

	Utf8Decoder::Utf8Decoder() noexcept : current_(end_of_file), next_(nullptr), last_(nullptr) { }

	Utf8Decoder::Utf8Decoder(std::string_view sv) noexcept : current_(0), next_(sv.data()), last_(sv.data() + sv.size())
	{
		if (next_ != last_)
		{
			++*this;
