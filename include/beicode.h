...
namespace beicode
{
	...
	{
		char buf[4] = {};
		int count = ::beicode::utf8_encode_code_point(buf, code_point);
	...
	*/
	struct Utf8Decoder
	{
		...
		BEICODE_EXPORT
		explicit Utf8Decoder(std::string_view buffer) noexcept;
		BEICODE_EXPORT
		Utf8Decoder(char const* first, char const* last) noexcept;
		...
		friend bool operator!=(const Utf8Decoder& lhs, const Utf8Decoder& rhs) noexcept { return !(lhs == rhs); }

		friend bool operator==(const Utf8Decoder& d, Utf8Decoder::sentinel) { return d.is_eof(); }
		friend bool operator==(Utf8Decoder::sentinel s, const Utf8Decoder& d) { return d == s; }
		friend bool operator!=(const Utf8Decoder& d, Utf8Decoder::sentinel) { return !d.is_eof(); }
		friend bool operator!=(Utf8Decoder::sentinel s, const Utf8Decoder& d) { return d != s; }
...
template <>
struct std::is_error_code_enum<beicode::utf8_errc> : std::true_type {};
